...
def _thead():
    return '<thead><tr class="group"><th>Fecha</th><th colspan="2">Residuo</th><th>Cant.</th><th colspan="2">Documentación</th><th colspan="5">Destino</th><th colspan="4">Ubic. destino</th><th>Transportista</th><th>Trat.</th><th>Proceso</th></tr><tr class="sub"><th>Fecha<br/>traslado</th><th>Cód.<br/>LER</th><th>Descripción</th><th class="num">(t)</th><th>Nº DI</th><th>Nº insc.<br/>transp.</th><th class="th-razon">Razón social</th><th>NIF</th><th>NIMA</th><th>Nº aut.</th><th>Tipo<br/>insc.</th><th>Mun.</th><th>Prov.</th><th>CCAA</th><th>País</th><th class="th-razon">Razón social</th><th>Método<br/>valor.</th><th>Proceso</th></tr></thead>'

def _row(r):
    return f'<tr><td class="date">{_e(r["fecha"])}</td><td class="ler">{_e(r["ler"])}</td><td>{_e(r["denominacion"])}</td><td class="num">{_fmt_t(r["cant_t"])}</td><td class="mono-long">{_e(r["di"])}</td><td class="mono-long">{_e(r["transp_insc"])}</td><td>{_e(r["dest_razon"])}</td><td class="mono-long">{_e(r["dest_nif"])}</td><td class="mono">{_e(r["dest_nima"])}</td><td class="mono-long">{_e(r["dest_aut"])}</td><td class="abbr">{_e(r["dest_tipo"])}</td><td>{_e(r["municipio"])}</td><td class="abbr">{_e(r["prov"])}</td><td class="abbr">{_e(r["ccaa"])}</td><td class="abbr">{_e(r["pais"])}</td><td>{_e(r["transp_razon"])}</td><td class="valor">{_e(r["metodo"])}</td><td class="valor">{_e(r["proceso"])}</td></tr>'
