...
def leer_datos_centro(wb):
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=1,max_row=30,values_only=True):
            ...
            elif 'tipo' in c: centro['tipo_aut']=_str(v)
...
def leer_salidas(wb):
    ...
    for row in ws.iter_rows(min_row=3,values_only=True):
        ...
        rows.append({'fecha':_fecha(cv(0)),'ler':_ler(cv(1)),'denominacion':_titulo(_str(cv(2))),'cant_t':peso_t,'di':_str(cv(5)),'ct':_str(cv(6)),'dest_razon':_titulo(_str(cv(7))),'dest_nif':_str(cv(8)),'dest_nima':_nima(cv(9)),'dest_aut':_str(cv(10)),'dest_tipo':_str(cv(11)),'municipio':_mun(cv(12)),'prov':_abbr(cv(13),PROV_ABBR),'ccaa':_abbr(cv(14),CCAA_ABBR),'pais':_abbr(cv(15),PAIS_ABBR),'transp_razon':_titulo(_str(cv(16))),'metodo':_str(cv(20)),'proceso':_str(cv(21))})
    return rows
...
def _colgroup():
    return '<colgroup><col style="width:5.3%"/><col style="width:3.3%"/><col style="width:7.1%"/><col style="width:3.6%"/><col style="width:6.5%"/><col style="width:10.1%"/><col style="width:9.5%"/><col style="width:4.8%"/><col style="width:5.7%"/><col style="width:5.3%"/><col style="width:2.7%"/><col style="width:5.0%"/><col style="width:2.6%"/><col style="width:2.4%"/><col style="width:2.3%"/><col style="width:9.5%"/><col style="width:7.2%"/><col style="width:7.2%"/></colgroup>'

def _thead():
    return '<thead><tr class="group"><th>Fecha</th><th colspan="2">Residuo</th><th>Cant.</th><th colspan="2">Documentación</th><th colspan="5">Destino</th><th colspan="4">Ubic. destino</th><th>Transportista</th><th>Trat.</th><th>Proceso</th></tr><tr class="sub"><th>Fecha<br/>traslado</th><th>Cód.<br/>LER</th><th>Descripción</th><th class="num">(t)</th><th>Nº DI</th><th>Nº CT</th><th class="th-razon">Razón social</th><th>NIF</th><th>NIMA</th><th>Nº aut.</th><th>Tipo<br/>insc.</th><th>Mun.</th><th>Prov.</th><th>CCAA</th><th>País</th><th class="th-razon">Razón social</th><th>Método<br/>valor.</th><th>Proceso</th></tr></thead>'

def _row(r):
    return f'<tr><td class="date">{_e(r["fecha"])}</td><td class="ler">{_e(r["ler"])}</td><td>{_e(r["denominacion"])}</td><td class="num">{_fmt_t(r["cant_t"])}</td><td class="mono-long">{_e(r["di"])}</td><td class="mono-long">{_e(r["ct"])}</td><td>{_e(r["dest_razon"])}</td><td class="mono-long">{_e(r["dest_nif"])}</td><td class="mono">{_e(r["dest_nima"])}</td><td class="mono-long">{_e(r["dest_aut"])}</td><td class="abbr">{_e(r["dest_tipo"])}</td><td>{_e(r["municipio"])}</td><td class="abbr">{_e(r["prov"])}</td><td class="abbr">{_e(r["ccaa"])}</td><td class="abbr">{_e(r["pais"])}</td><td>{_e(r["transp_razon"])}</td><td class="valor">{_e(r["metodo"])}</td><td class="valor">{_e(r["proceso"])}</td></tr>'
